...
LOGLEVEL="info"
# Set dedicated home directory for the ./polard instance
HOMEDIR="./nodes/node0/polard"
# to trace evm
#TRACE="--trace"
TRACE=""
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	## 1. Back to `./polard keys add` step, init more keys
	## 2. Back to `./polard add-genesis-account` step, add balance for those
	## 3. Clone this ~/../polard home directory into some others, let's say `~/.cloned./polard`
	## 4. Run `gentx` in each of those folders
	## 5. Copy the `gentx-*` folders under `~/.cloned./polard/config/gentx/` folders into the original `~/../polard/config/gentx`
...
#./polard start --pruning=nothing --api.enabled-unsafe-cors --api.enable --api.swagger --minimum-gas-prices=0.0001abera --home "$HOMEDIR"