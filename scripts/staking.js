...
let run = async function () {
  try {
    ...
    const fixed = `--pubkey=${pubkey} --moniker="node1" --commission-rate="0.05" --commission-max-rate="0.10" --commission-max-change-rate="0.01" --min-self-delegation="1" --gas="600000" --from=${from} --home=./nodes/node1/evmosd/ --keyring-backend=test --chain-id=evmos_20191205-1 -y`;
    let cmd;
    let reply;
