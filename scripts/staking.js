...
let run = async function () {
  try {
    ...
    const fixed = `--pubkey=${pubkey} --moniker="node4" --commission-rate="0.05" --commission-max-rate="0.10" --commission-max-change-rate="0.01" --min-self-delegation="100000000000000000000" --gas="600000" --from=${from} --home=./nodes/node4/evmosd/ --keyring-backend=test --chain-id=quarix_8888888-1 -y`;
    let cmd;
    let reply;

    {
      ...
      // cmd = `./evmosd tx staking create-validator --amount=100000000000000000000aqare ${fixed}`;
      // reply = await execPromis(cmd, { cwd });
      // console.log(cmd, "\n", decodeReply(reply));
      //
      // await sleep(1000 * 10);

      cmd = `./evmosd tx staking create-validator --amount=100000000000000000000aqrx ${fixed}`;
      reply = await execPromis(cmd, { cwd });
      console.log(cmd, "\n", decodeReply(reply));
    }
  } catch (error) {
